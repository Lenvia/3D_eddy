...
def eddy_detection(lon,lat,depth,uvel,vvel,day,R2_criterion,OW_start,max_evaluation_points,min_eddie_cells):
    ...
    uvel = np.expand_dims(uvel, 1)
    vvel = np.expand_dims(vvel, 1)
    ...
    # x[i, j]似乎是lon[i], lat[j]位置到经纬度为(0, 0)的经度距离
    for i in range(0,nx):
        for j in range(0,ny):
            x[i,j] = 2.*math.pi*R*math.cos(lat[j]*math.pi/180.)*lon[i]/360.  # 经度距离
            y[i,j] = 2.*math.pi*R*lat[j]/360.

    print(x)
    print(y)
    # Gridcell area
    # dx是相邻点经度距离数组（起始点计算有点特殊）
    # dy同理
    # grid_area(i,j)是以改点为顶点，的一个小区域的面积
    ...
    # OW_eddies值为1的位置可能是涡旋区域
    OW_eddies = np.zeros(OW.shape, dtype=int)
    OW_eddies[np.where(OW < OW_start)] = 1  # np.where返回的是索引
    ...
    # local_mins返回的是shape为(3, max_evaluation_points)的数组。
    # local_mins[0][i], local_mins[1][i], local_mins[2][i]分别表示第i个点所在的经度、纬度、层数的索引
    local_mins = local_peaks(OW, OW_start, max_evaluation_points)
    num_mins = local_mins.shape[1]  # 点的个数
    ...
    for imin in range(0, num_mins):
        ...
        # 每次搜索都会重置eddie_mask，但all_eddie_mask不重置
        eddie_mask = np.zeros(uvel.shape,dtype=int)
        neighbor_mask = np.zeros(uvel.shape,dtype=int)

        eddie_mask[ie, je, ke] = 1
        minOW = np.zeros((max_eddy_cells_search, 1))  # 邻近区域的OW值
        volume = np.zeros((max_eddy_cells_search, 1))  # 邻近区域体积
        R2 = np.zeros((max_eddy_cells_search, 1))

        minOW[0] = OW[ie, je, ke]
        volume[0] = grid_area[ie, je]* dz[ke]  # 体积
        start_checking = 0

        # 随着max_k和min_k逐渐更新，最终得到涡旋所在的区间
        max_k = 0
        min_k = nz
        ...
        for ind in range(1, max_eddy_cells_search):
            ...
            # 东西南北上下6个方向，neighbor_mask相应位置对eddi_mask去反，如果没被搜索过，则nei_mask对应位置为1
            ...
            minOW[ind] = np.min(OW[neighbor_indices])  # 找出候选网格里OW最小的点
            minInd = np.where(OW[neighbor_indices] == minOW[ind])[0][0]
            ie,je,ke =np.asarray(neighbor_indices)[:,minInd]  # 最小点在位置的索引
            ...
            minOW[ind] = np.max((minOW[ind], minOW[ind - 1]))  # 本涡旋的综合OW值取所有小块中的最大值
            volume[ind] = volume[ind - 1] + grid_area[ie, je]*dz[ke]  # 增加这一小块的体积
            ...
            # 此说明性算法使用第一个被接受的涡流，并且相同单元格中的所有后续涡流都是重复的。
            if np.mod(ind, 20) == 0:
                if np.max(eddie_mask + all_eddies_mask) == 2:  # 两个涡旋有重叠
                    print('No, duplicate\n')
                    break
            if start_checking == 0:
            ...
            # is met.
                ...
                # Compute R2 value of linear fit of volume versus min OW.
                    temp = np.corrcoef(minOW[0:ind+1],volume[0:ind+1],rowvar=False)  # 相关系数矩阵
                    R2[ind] = temp[0, 1]
                ...
                if R2[ind] < R2_criterion:
                    ...
                    minOW_eddie = np.min(OW[eddie_indices])  # 该涡旋区域内最小的OW值
                    tempInd = np.where(OW[eddie_indices] == minOW_eddie)[0][0]
                    iE, jE, kE = np.asarray(eddie_indices)[:,tempInd]  # 最小OW值所在的位置索引
...
# Using the scipy.signal.fing_peaks function with flattened array and unravelling it, probably much faster.
    A_flat = A.flatten()
    peaks = sg.find_peaks(-A_flat,height=-A_start)  # height: 低于指定height的信号都不考虑
    # print(peaks)
    n_minima = len(peaks[0])  # 峰值个数
    # print(n_minima)
    local_min = np.asarray(np.unravel_index(peaks[0], A.shape))  # 获取一个/组int类型的索引值在一个多维数组中的位置。
    # print(local_min)  # 位置（经度、纬度、层）
    # print(local_min.shape)  # (3, 2391)  3表示三个位置纬度，2391为点的个数
    # print(local_min.shape[1])  # 2391
    # 返回size个在[0, 2391)内的随机数
    sample = np.random.randint(0,local_min.shape[1], size=np.min((max_evaluation_points, n_minima)))
    # print(sample)

    return local_min[:, sample]  # 返回local_min中的部分列，列数在sample中
...
if __name__ == '__main__':
    ...
    lon, lat, uvel, vvel, vorticity, OW, OW_eddies, eddie_census, nEddies, circulation_mask, levels = eddy_detection(
        lon, lat, depth, uvel[:,0,:,:], vvel[:,0,:,:], day, R2_criterion, OW_start, max_evaluation_points, min_eddie_cells)
