...
function stateMap(){
    for(let day=0; day<stepLimit; day++){
        for(let index = 0; index<eddyBackwards[day].length; index++){
            var state = String(day)+'-'+String(index);
            Find(state);
        }
    }
}
...
function Find(state){
    if(state2identifier.get(state) != undefined){  // 已经有了 不用找饿了
        return state2identifier.get(state);
    }
    else{
        var day = parseInt(state.split("-")[0]);
        var index = parseInt(state.split("-")[1]);

        if(eddyBackwards[day][index].length == 0){
            state2identifier.set(state, name2identifier.get(state));  // 如果没有前驱，它一定是一个涡旋的开始，一定能从name2identifier里找到
            return state2identifier.get(state);
        }
        else{
            var pre = eddyBackwards[day][index][0];  // 只找第一个前驱
            state2identifier.set(state, Find(pre));
            return state2identifier.get(state);
        }
    }
}
    ...
    $.ajax({
        ...
        success: function(res) { //请求成功完成后要执行的方法 
            ...
            for(var num=0; num<liveInfo.length; num++){
                var curName = liveInfo[num]['name'];
                name2identifier.set(curName, num);
                identifier2name.set(num, curName);
            }
            eddyCount = liveInfo.length;
            for (var i =0; i<eddyCount; i++){
                eddyIdentifiers.push(i);
            }
        }
    })
}
